...
class WhatsAppConfig:
    ...
    # Embedded Signup — lets a tenant connect their OWN number via a Facebook
    # popup (see core/oauth/views.py:whatsapp_embedded_signup) instead of
    # everyone sharing the single ACCESS_TOKEN/PHONE_NUMBER_ID above.
    APP_ID = os.environ.get("WHATSAPP_APP_ID", "")
    CONFIG_ID = os.environ.get("WHATSAPP_CONFIG_ID", "")
    ...
    @classmethod
    def embedded_signup_configured(cls) -> bool:
        """Whether the popup connect flow can run at all (app-level, not per-tenant)."""
        return bool(cls.APP_ID and cls.CONFIG_ID and cls.APP_SECRET)

    @classmethod
    def resolve(cls, connection=None):
        """
        Per-tenant credentials from a completed Embedded Signup win over the
        single global ACCESS_TOKEN/PHONE_NUMBER_ID in .env. APP_SECRET and
        VERIFY_TOKEN stay app-level — one Meta app can host many tenants'
        WABAs (that's the point of Embedded Signup), and webhook signature
        verification is keyed on the app secret, not any one tenant's token.
        """
        tokens = getattr(connection, "oauth_tokens", None) or {}
        access_token = tokens.get("access_token") or cls.ACCESS_TOKEN
        phone_number_id = tokens.get("phone_number_id") or cls.PHONE_NUMBER_ID
        waba_id = tokens.get("waba_id") or cls.WABA_ID
        return SimpleNamespace(
            ...
            GRAPH_API_VERSION=cls.GRAPH_API_VERSION,
            GRAPH_BASE_URL=cls.GRAPH_BASE_URL,
            is_configured=lambda: bool(access_token and phone_number_id),
        )
...
class WhatsAppClient:
    ...
    def __init__(self, connection=None):
        self.cfg = WhatsAppConfig.resolve(connection)
...
class WhatsAppAdapter:
    ...
    def send_reply(self, interaction, text: str) -> SendResult:
        ...
        try:
            result = WhatsAppClient(self.connection).send_text_message(to=to, body=text)
